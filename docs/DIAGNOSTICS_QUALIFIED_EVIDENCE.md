...
5. Choose **Open Windows Recycle Bin** if useful, then manually select `RECYCLE-ME.png` and choose Windows **Restore**. This button only opens the Windows Recycle Bin shell view; it never restores an item itself.
...
- rejected if the runtime/build contract changes before move or verification;
...
The **Open Windows Recycle Bin** helper is deliberately navigation-only. It is enabled only while the active generated session is in the `recycled` stage and opens the fixed Windows `shell:RecycleBinFolder` namespace. It does not select an item, perform Restore, scan personal Recycle Bin content or weaken the explicit human-action requirement.
...
4. A `prepared` session re-enables only the guarded move step; a `recycled` session re-enables the navigation-only **Open Windows Recycle Bin** helper plus restore verification; a `restored-verified` session revalidates an existing report or enables safe report regeneration when the report is missing.
...
The GUI does **not** restore files automatically, edit `STATUS.md`, mark the 1.0 gate complete or publish a release. **Open Windows Recycle Bin** only navigates to the system Recycle Bin; the user must still perform Windows Restore manually. Both the evidence report and packaged attestation keep `acceptance_gate_closed: false`; genuine physical Windows evidence still requires manual acceptance review before the repository checklist can change.
