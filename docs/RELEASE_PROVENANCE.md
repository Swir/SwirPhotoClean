...
The standalone `python tools/release_evidence.py verify --file ...` path is fail-closed at filesystem intake, not only at JSON/schema validation. It accepts one bounded regular single-link file, rejects symlink/junction/reparse and hardlink aliases, binds the inspected path to the opened file handle, detects metadata/identity changes during the read, rechecks the path afterwards, and parses only the bytes captured from that verified handle. This keeps manual release review from silently validating a different or swapped evidence object than the one that was inspected.

The packaged attestation handoff is transactional as well. `SwirPhotoClean.exe --recycle-restore-attest` refuses an output path that aliases the source evidence report, manifest, or generated KEEP/RECYCLE fixture, including existing hardlink aliases. It also rejects an existing symlink, junction/reparse point or non-regular output. The writer records the current output identity, stages JSON under an unpredictable exclusive temporary name in the destination directory, validates and fsyncs those staged bytes, rechecks that the destination did not change underneath it, and only then performs one atomic replacement. A stale predictable `.tmp` file is never reused, so interrupted or hostile staging state cannot silently overwrite the evidence inputs or redirect the qualified attestation.
