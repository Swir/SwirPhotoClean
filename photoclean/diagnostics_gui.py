...
DIAG_EN = {
    ...
    "Kopia trafiła do Kosza, a oryginał pozostał na miejscu. Kliknij „Otwórz Kosz Windows”, przywróć RECYCLE-ME.png ręcznie, a potem kliknij „Sprawdź przywróconą kopię”.": "The copy reached the Recycle Bin and the original stayed in place. Click “Open Windows Recycle Bin”, manually restore RECYCLE-ME.png, then click “Verify restored copy”.",
...
class DiagnosticsWindow:
    ...
    def _build_recycle(self, parent):
        ...
        self.open_button = ttk.Button(
            secondary,
            text=diag_tr("Otwórz folder testu"),
        ...
        self.recycle_bin_button = ttk.Button(
            secondary,
            text=diag_tr("Otwórz Kosz Windows"),
            command=self.open_recycle_bin,
            state="disabled",
        )
        self.recycle_bin_button.pack(side="left")
    ...
    def _apply_check_state(self, check: RecycleVerification, report: Path | None = None):
        ...
        self.recycle_bin_button.configure(
            state="normal" if check.stage == "recycled" else "disabled"
        )
        ...
        if check.stage == "recycled":
            ...
            self.recycle_status.set(
                diag_tr(
                    "Kopia trafiła do Kosza, a oryginał pozostał na miejscu. Kliknij „Otwórz Kosz Windows”, przywróć RECYCLE-ME.png ręcznie, a potem kliknij „Sprawdź przywróconą kopię”."
                )
            )
            return
    ...
    def open_recycle_bin(self):
        """Open the Windows Recycle Bin for the one required manual Restore action.

        This helper is deliberately navigation-only: it never restores an item and
        is enabled only while the generated evidence session is in ``recycled``
        stage. The user must still choose RECYCLE-ME.png and invoke Windows Restore.
        """
        if self.check is None or self.check.stage != "recycled":
            return
        try:
            if os.name != "nt":
                raise OSError("Windows required")
            ...
            messagebox.showerror(
                diag_tr("Nie można otworzyć Kosza Windows"),
                str(error),
                parent=self.window,
            )
