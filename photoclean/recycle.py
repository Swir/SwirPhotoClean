...
def recycle_file(path):
    if os.name != "nt":
        raise OSError("Przenoszenie do kosza w tej wersji jest dostępne tylko na Windows.")
    import ctypes
    import pythoncom
    ...
    if not ctypes.windll.kernel32.GetVolumePathNameW(absolute, volume, len(volume)):
        raise OSError("Nie można ustalić woluminu pliku.")
    if ctypes.windll.kernel32.GetDriveTypeW(volume.value) != 3:
        raise OSError("Kosz obsługujemy tylko na lokalnych dyskach stałych. Dysk sieciowy lub wymienny: operacja zablokowana.")
    ...
    class RecycleOnlySink(FileOperationProgressSink):
        ...
        def PreDeleteItem(self, flags, item):
            if not flags & shellcon.TSF_DELETE_RECYCLE_IF_POSSIBLE:
                self.error = "Windows nie potwierdził dostępności kosza; plik pozostawiono na miejscu."
                raise COMException(desc="Trwałe usuwanie jest zabronione", scode=-2147467260)
            return 0
